...
class CGCN(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(CGCN, self).__init__(config, dataset)
    ...
    def acquire_classes(self):
        ...
        user_classes = [member_ship[i][0] for i in range(self.n_users)]
        item_classes = [member_ship[self.n_users + i][0] for i in range(self.n_items)]
        ...
        return torch.tensor(user_classes, dtype=torch.long).to(self.device), torch.tensor(item_classes, dtype=torch.long).to(self.device), len(classes_set)
    ...
    def calculate_loss(self, interaction):
        ...
        return mf_loss + self.reg_weight * reg_loss, self.class_loss(center_embedding, user, pos_item)

    def class_loss(self, current_embedding, user, item):
        current_user_embeddings, current_item_embeddings = torch.split(
            current_embedding, [self.n_users, self.n_items]
        )
        return self.class_weight * (self.user_criterion(self.mlp1(current_user_embeddings[user]), self.user_classes[user]) + \
