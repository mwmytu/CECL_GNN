...
class ConstrastComponent(GeneralRecommender):
    ...
    def __init__(self, config, dataset, test_func=None):
        ...
        if 'classification' in config:
            self.enable_classification = config['classification']
        else:
            self.enable_classification = True
        if 'contrast' in config:
            self.enable_contrast = config['contrast']
        else:
        ...
        if self.num_clusters == -1 and (self.enable_classification or self.enable_contrast):
            self.user_classes, self.item_classes, self.classes = self.scd()
            self.create_neighbor_matrix()
            print(f"class:{self.classes}")
        elif self.num_clusters and (self.enable_classification or self.enable_contrast):
    ...
    def create_neighbor_matrix(self):
        ...
        self.user_neighbor_matrix = local_cache(self.dataset + "-user_neighbor_matrix",
                                                lambda: self.acquire_neighbor_matrix_by_class(uc,
                                                                                              self.n_users))
        self.item_neighbor_matrix = local_cache(self.dataset + "-item_neighbor_matrix",
                                                lambda: self.acquire_neighbor_matrix_by_class(ic,
                                                                                              self.n_items))
    ...
    def class_loss(self, current_embedding, user, item):
        ...
        return self.class_weight * (
                self.user_criterion(self.mlp1(current_user_embeddings[user]),
                                    self.user_classes[user]) + \
                self.item_criterion(self.mlp2(current_item_embeddings[item]),
    ...
    def scd(self):
        ...
        for i in range(self.interaction_matrix.getnnz()):
            G.add_edge(self.interaction_matrix.col[i] + self.n_users,
                       self.interaction_matrix.row[i])
        ...
        return self.convert_classes(
            torch.tensor(user_classes, dtype=torch.long)), self.convert_classes(
...
if __name__ == '__main__':
    ...
    class Dataset:
        def inter_matrix(self, form):
            ...
            arr = [(0, 0), (0, 2), (0, 4), (1, 0), (1, 2), (1, 4), (2, 1), (2, 4), (3, 4), (3, 1),
                   (4, 0), (4, 4)]
            for (i, j) in arr:
                ret[i, j] = 1
            L = sp.coo_matrix(ret)
